...
def process_scene(i, scene):
    ...
    # 2. Video Validation Loop
    vid_url = fetch_pexels_video(scene.get('keyword', 'technology'))
    is_valid = False
    if vid_url:
        try:
            r = requests.get(vid_url, timeout=30)
            with open(raw_video, "wb") as f: f.write(r.content)
            # FFprobe check for file integrity
            check = subprocess.run(['ffprobe', raw_video], capture_output=True)
            if check.returncode == 0: is_valid = True
        except: is_valid = False
    
    # 3. Final Render (With Fallback)
    if is_valid:
...
# Use -async 1 to prevent audio/video drift
...
requests.post(f"https://api.telegram.org/bot{bot_token}/sendMessage", json={"chat_id": chat_id, "text": f"✅ Video ready! {video_link}"})
