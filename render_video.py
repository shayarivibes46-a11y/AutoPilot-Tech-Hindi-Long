...
def fetch_pexels_video(keyword):
    """Fetch video and verify URL exists"""
    try:
...
def process_scene(i, scene):
    ...
    if vid_url:
        subprocess.run(['ffmpeg', '-y', '-i', vid_url, '-t', str(dur), '-vf', 'scale=1280:720:force_original_aspect_ratio=increase,crop=1280:720,fps=25', '-c:v', 'libx264', '-preset', 'ultrafast', '-pix_fmt', 'yuv420p', '-an', scene_filename], check=True)
    else:
        # Fallback to a RED screen if video fails (to debug)
        subprocess.run(['ffmpeg', '-y', '-f', 'lavfi', '-i', 'color=c=red:s=1280x720', '-t', str(dur), '-c:v', 'libx264', '-pix_fmt', 'yuv420p', '-an', scene_filename], check=True)
