...
def process_scene(i, scene):
    ...
    # 2. Stable Render (1280x720, forced)
...
# --- FINAL SYNC MERGE ---
# -shortest flag aur -async 1 add kiya hai taaki audio/video ka end point match ho
subprocess.run([
    'ffmpeg', '-y', '-i', 'v_merged.mp4', '-i', 'a_merged.wav', '-stream_loop', '-1', '-i', 'bgm.mp3',
    '-filter_complex', '[1:a]loudnorm[a_v];[2:a]volume=0.05[a_bgm];[a_v][a_bgm]amix=inputs=2:duration=first[aout];[0:v]drawtext=text=\'Engineering Decode\':x=w-tw-50:y=h-th-50:fontsize=48:fontcolor=white@0.5[vout]',
    '-map', '[vout]', '-map', '[aout]', '-c:v', 'libx264', '-preset', 'slow', '-crf', '18', '-pix_fmt', 'yuv420p', '-c:a', 'aac', '-async', '1', '-shortest', 'final_video.mp4'
], check=True)
