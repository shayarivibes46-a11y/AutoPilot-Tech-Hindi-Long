...
def process_scene(i, scene):
    ...
    # 2. Normalize Video (Fixes Black Screen/Codec mismatch)
    ...
    # Force 1280x720, 25fps for stable concatenation
    subprocess.run([
        'ffmpeg', '-y', '-f', 'lavfi', '-i', 'color=c=black:s=1280x720:d='+str(dur),
        '-vf', 'fps=25,format=yuv420p', '-c:v', 'libx264', '-preset', 'ultrafast', norm_filename
    ], check=True)
    
    return {"vid": norm_filename, "aud": audio_path, "index": i}
...
subprocess.run([
    ...
    '-filter_complex', '[1:a]loudnorm[a_norm];[a_norm][2:a]amix=inputs=2:duration=first[aout];[0:v]drawtext=text=\'Engineering Decode\':x=w-tw-50:y=h-th-50:fontsize=48:fontcolor=white@0.5[vout]',
