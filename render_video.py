...
def process_scene(i, scene):
    ...
    # 2. Black Screen Fallback (Stable Render)
...
# --- EXECUTION ---
results = []  # <--- Initialize kiya taaki NameError na aaye
...
# Final Merge
subprocess.run([
    'ffmpeg', '-y', '-i', 'v_merged.mp4', '-i', 'a_merged.wav', '-filter_complex', '[0:v]drawtext=text=\'Engineering Decode\':x=w-tw-50:y=h-th-50:fontsize=48:fontcolor=white@0.5[vout]', '-map', '[vout]', '-map', '1:a', '-c:v', 'libx264', '-c:a', 'aac', 'final_video.mp4'
], check=True)
...
try:
    ...
    requests.post(f"https://api.telegram.org/bot{bot_token}/sendMessage", json={"chat_id": chat_id, "text": f"✅ Video ready! {link}"})
except: pass
