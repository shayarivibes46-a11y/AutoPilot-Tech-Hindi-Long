...
def clean_text_for_tts(text):
    # Strict sanitization: Sirf allowed characters bacheinge
...
def process_scene(i, scene):
    ...
    try:
        ...
        # Pexels Video Fetch (Placeholder for fetch logic)
        ...
        subprocess.run(['ffmpeg', '-y', '-f', 'lavfi', '-i', 'color=c=black:s=1280x720', '-t', str(dur), '-vf', vf_filter, '-c:v', 'libx264', '-preset', 'ultrafast', '-pix_fmt', 'yuv420p', '-an', scene_filename], check=True)
...
# Final Merge with corrected asplit=2 syntax to prevent unconnected output error
subprocess.run([
